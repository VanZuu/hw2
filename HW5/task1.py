...
complexity = {'легко': (1, 7, 10), 'средний': (3, 7, 8), 'тяжелый': (5, 9, 10)}
for i in complexity:
    os.system('CLS')
    a, b, c = complexity[i]
    width, height = b, b
    ...
    for j in range(a):
        while True:
            ...
            if check_ships_around(field, d, e, 1):
                field[d][e] = 1
                break
    print(f'Ты начал игру на  {i} сложности! У тебя есть {a} враги и {c} выстрелов. Удачки!')
    count = 0
    for j in range(c):
        print(f'У тебя есть {c - j} выстрелов, {j} было сделано.')
        while True:
            print_field(field, width, height, True)
            shoot = input('Введите свой выбор, разделенный запятыми строку и ячейку("0,0" or "10,10"): ')
            os.system('CLS')
            shoot_i, shoot_j = shoot.split(',')
            shoot_i, shoot_j = int(shoot_i), int(shoot_j)
            if (0 < shoot_i < height) and (0 < shoot_j < width): break
            else: print('Ты ввел неправильную ячейку!')
        if field[shoot_i][shoot_j] == 1:
            field[shoot_i][shoot_j] = 2
            count += 1
        else: field[shoot_i][shoot_j] = 3
        if count == a:
            print(f'Ты выйграешь при {j + 1} выстрелах!')
